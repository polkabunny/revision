...
Q1 (a)
Explain how Artificual Intelligence can be considered as a form of search. Briefly explain what a search approach to solving a problem would look like.
...
```
    ...
    A search approach is based on using an algorithm with random combinations of parameters, and repeating until exhaustive search of the space has been completed. However, in cases where the space is too large, this cannot be done exhaustively, so a heuristic is more suitable.
```
...
######Answer:
```
    If x is a random value generated by the heuristic, then you may not actually be able to find the global optimum or the local optimum. The best method for finding the global optimum is to find the slope for it, and then use exemplars from this to reach the global optimum. However, this method does not utilise exemplars, so will likely never find it, unless by pure chance.
```
...
######Answer:
```
    No, heuristics differ ased on the algorithm they use - the one in (c) does not use exemplars, while another heuristic may incorporate exemplars, leading to possibly different results.
```
...
######Answer:
```
    A metaheuristic is a collection of design patterns and general algorithms that can be applied to a variety of problems, without domain-specific knowledge. (Jack of all trades approach to problem solving.)
```
