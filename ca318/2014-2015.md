...
######Answer:
```
A metaheuristic is used to find a sufficiently good solution, with no domain-specific knowledge. However it is not random, it uses design patterns and general algorithms that can be applied to a number of problems, so can be used for a variety of problems.
```
