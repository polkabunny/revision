...
![Possible input](https://scontent.xx.fbcdn.net/hphotos-xap1/v/t1.0-9/1915361_10208824611400391_8713298316572290233_n.jpg?oh=aa99a30269547ce750bae62f8736218c&oe=570C0A76)
```
...
```
   ...
   (c)
We are trying to learn to predicty horse races from exemplars. Among our exemplars, we find the following:
...
######Answer:
```
You wouldn't get any new solutions after the first run with it in place
Excluding it would keep new solutions
```

iv) In steop 1 in the loop, discuss how "Pick the best 1" would compare with "Pick the best 18".

######Answer:
```
"Pick the best 1"  - leads to only slight variants early on, less diverse as only the best in the first generation would be picked
"Pick the best 18" - spending too much time on poor solutions that are carried on and modified, even though best are still carried
```

   (c)
Consider an algorithm for mate choise as follows:
